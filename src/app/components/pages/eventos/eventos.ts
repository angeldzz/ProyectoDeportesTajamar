...
const API_BASE = 'https://apideportestajamar.azurewebsites.net/api/Eventos';
...
export class EventosComponent implements OnInit {
  ...
  async loadEventos() {
    ...
    try {
      ...
      // normalize items to expected fields used in template
  ...
  async crearEvento() {
    this.error = null;
    try {
      // API defines create endpoint as POST /api/Eventos/create/{datofecha}
      const datofecha = this.form.fechaEvento || '';
      const res = await fetch(`${API_BASE}/create/${encodeURIComponent(datofecha)}`, { method: 'POST' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  async actualizarEvento() {
    this.error = null;
    try {
      const body = JSON.stringify({ idEvento: this.form.idEvento, fechaEvento: this.form.fechaEvento, nombreEvento: this.form.titulo, lugar: this.form.lugar, descripcion: this.form.descripcion });
      const res = await fetch(`${API_BASE}/update`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      await this.loadEventos();
      this.clearSelection();
    } catch (err: any) {
